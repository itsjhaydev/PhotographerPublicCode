...
{
    return [self URLForQuery:@"https://api.flickr.com/services/rest/?method=flickr.places.getTopPlacesList&place_type_id=29"];
}

+ (NSURL *)URLforImagesFromUserID:(id)userId

{
    return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.people.getPhotos&user_id=%@&per_page=50",userId]];
}
...
{
    return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.getRecent&per_page=50"]];
}
...
{
    return [self URLForQuery:[NSString stringWithFormat:@"https://www.flickr.com/services/rest/?method=flickr.photos.getInfo&photo_id=%@&secret=%@", photoId,photoSecret]];
}

+ (NSURL *)URLforPhotosAtLat:(NSNumber*)latitude andLong:(NSNumber*)longitude{
   return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.search&license=1,2,4,7&has_geo=1&lat=%f&lon=%f&extras=original_format,description,geo,date_upload,owner_name",[latitude floatValue],[longitude floatValue]]];
}
...
{
    return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.people.getInfo&user_id=%@", flickrUserId]];
}
